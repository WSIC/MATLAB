...
%% Test the input parameters.
    if nargin<4 || isempty(sourceImg) || isempty(targetImg) || ...
            isempty(sourceHdr) || isempty(targetHdr) % Both datasets should be complete
        help alignDicoms
        error('Not enough input arguments! Min. 4 (non-empty) input vars required.');   % Error.
    elseif nargin==4                            
        interp = 'cubic';   
        fill = 0;
    elseif nargin==5    
...
function M = TransMatrix(info,ND)
    ...
    ipp = ipp - r*ps(2) - c*ps(1) - s*ps(3);  % This is important when pixeldimensions are not equal.

    Tipp = [1,     0,     0,     ipp(1); ...  % Translation
            0,     1,     0,     ipp(2); ...
            0,     0,     1,     ipp(3); ...
            0,     0,     0,     1];
